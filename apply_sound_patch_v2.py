#!/usr/bin/env python3
"""
Arena_Battle sound patcher V2

사용법:
1) 이 파일과 sounds 폴더를 Arena_Battle 프로젝트 폴더(main.py 있는 곳)에 넣기
2) 터미널에서 실행:
   python apply_sound_patch_v2.py
   또는
   py apply_sound_patch_v2.py
3) 생성/수정된 main.py 실행:
   python main.py

이미 예전 사운드 패치를 적용한 상태라면 BGM/효과음 볼륨만 V2 기준으로 낮춰줍니다.
...
def tune_existing_sound_patch(text):
    ...
    text = re.sub(r'pygame\.mixer\.music\.set_volume\([0-9.]+\)', 'pygame.mixer.music.set_volume(0.10)', text)
    replacements = {
...
def main():
    if not MAIN_FILE.exists():
        fail("현재 폴더에 main.py가 없습니다. apply_sound_patch_v2.py를 main.py와 같은 폴더로 옮겨주세요.")
    ...
    if PATCH_MARK in text:
        ...
        print("완료: 이미 적용된 사운드 패치를 V2 볼륨 기준으로 조정했습니다.")
        print("sounds 폴더도 V2 버전으로 교체해 주세요.")
        return
    ...
    MAIN_FILE.write_text(text, encoding="utf-8", newline="\n")
    print("완료: main.py에 V2 사운드 기능을 추가했습니다.")
